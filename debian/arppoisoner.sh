...
requirements() {
	...
	# Note: Need to use sh -c to execute the redirection as root
	...
	do
		case $opt in
			"Poison")
				...
				# --- FIXED POISONING COMMANDS (Removed leading spaces) ---
				echo "Stop sniffing...U addicted as hell! (Check the two new windows)"
				# Run the first arpspoof in a separate xterm and background it
				sudo xterm -e arpspoof -i $inter -t $vict -r $gate &
				# Run the second arpspoof in a separate xterm and background it
